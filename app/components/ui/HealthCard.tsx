...
export default function HealthCard({ healthFactor }: HealthCardProps) {
  ...
  return (
    <Card className={`rounded-[var(--radius-xl)] border-white/20 bg-gradient-to-br ${healthState.bgGradient} p-6 backdrop-blur-xl dark:border-gray-700/50 shadow-[var(--shadow-md)]`}>
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          <Badge className={`rounded-xl ${healthState.color} backdrop-blur-sm`}>{healthState.status}</Badge>
        </div>
      </div>
